...
def download_dcm(bucket_name, slice_id, save_path):
    '''
    :param bucket_name:文件存储的桶名称,即文件存储的上层目录名,非全路径
    :param slice_id:病人的序列id号,即文件名,非全路径
    :param save_path:本地存储位置
    :return:
    '''
    s3_keys = {"user": "yanjiuyuan",
               "access_key": "XVFQNEV4E4M95O5OQNNX",
               "secret_key": "99wLC2m1fuj9NinTa87tBaNCkdD2OvBXnaOffLMl"}

    host = "10.50.60.15"
    port = 7480

    conn = S3Connection(s3_keys["access_key"], s3_keys["secret_key"],
                        host=host, port=port, is_secure=False,
                        calling_format=boto.s3.connection.OrdinaryCallingFormat())
    if len(slice_id)>0:
        img_bucket = conn.get_bucket(bucket_name)
        for filename in slice_id:
            dcm_key = img_bucket.get_key(filename)
            save_name = os.path.join(save_path, os.path.split(filename)[1])
            dcm_key.get_contents_to_filename(save_name)
...
def get_patient_ct_data_mask_info(dcm_path,opt):
